...
/obj/structure/railing
	...
	icon = 'sephora/icons/obj/structures/railing.dmi'
	density = TRUE
	climbable = TRUE
	anchored = TRUE
	icon_state = "railing0"
...
/obj/structure/railing/update_icon(var/UpdateNeighgors = 1)
	...
	if (!check || !anchored)//|| !anchored
		...
		//левая сторона
		if (check & 32)
			overlays += image ('sephora/icons/obj/structures/railing.dmi', src, "corneroverlay")
		if ((check & 16) || !(check & 32) || (check & 64))
			overlays += image ('sephora/icons/obj/structures/railing.dmi', src, "frontoverlay_l")
		if (!(check & 2) || (check & 1) || (check & 4))
			overlays += image ('sephora/icons/obj/structures/railing.dmi', src, "frontoverlay_r")
			if(check & 4)
				switch (src.dir)
					if (NORTH)
						overlays += image ('sephora/icons/obj/structures/railing.dmi', src, "mcorneroverlay", pixel_x = 32)
					if (SOUTH)
						overlays += image ('sephora/icons/obj/structures/railing.dmi', src, "mcorneroverlay", pixel_x = -32)
					if (EAST)
						overlays += image ('sephora/icons/obj/structures/railing.dmi', src, "mcorneroverlay", pixel_y = -32)
					if (WEST)
						overlays += image ('sephora/icons/obj/structures/railing.dmi', src, "mcorneroverlay", pixel_y = 32)
