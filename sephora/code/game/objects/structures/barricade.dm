/obj/structure/peacekeeper_barricade //CREDIT TO CM FOR THIS. Cleanup up by Kmc.
	icon = 'sephora/icons/obj/barricades.dmi'
	climbable = TRUE
	anchored = TRUE
	density = TRUE
...
/obj/structure/peacekeeper_barricade/metal/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/stack/barbed_wire))
		...
		playsound(loc, 'sephora/sound/effects/barbed_wire_movement.ogg', 25, 1)
		user.visible_message("<span class='notice'>[user] sets up [I] on [src].</span>",
		"<span class='notice'>You set up [I] on [src].</span>")

		if(!closed)
			wired_overlay = image('sephora/icons/obj/barricades.dmi', icon_state = "[barricade_type]_wire")
		else
			wired_overlay = image('sephora/icons/obj/barricades.dmi', icon_state = "[barricade_type]_closed_wire")
...
/obj/structure/peacekeeper_barricade/proc/update_overlay()
	...
	overlays -= wired_overlay
	if(!closed)
		wired_overlay = image('sephora/icons/obj/barricades.dmi', icon_state = "[src.barricade_type]_wire")
	else
		wired_overlay = image('sephora/icons/obj/barricades.dmi', icon_state = "[src.barricade_type]_closed_wire")
	overlays += wired_overlay
