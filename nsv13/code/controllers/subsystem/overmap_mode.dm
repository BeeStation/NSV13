...
/datum/overmap_gamemode/proc/check_completion() //This gets called by checking the communication console/modcomp program + automatically once every 10 minutes
	...
	if(istype(SSticker.mode, /datum/game_mode/pvp)) //If the gamemode is PVP and a faction has over a thousand points, they win.
		for(var/X in SSstar_system.factions)
			var/datum/game_mode/pvp/mode = SSticker.mode
			var/datum/faction/F = X
			if(F.tickets >= SSstar_system.tickets_to_win)
