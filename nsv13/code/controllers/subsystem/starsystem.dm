...
#define NONRELAXATION_PENALTY 1.6 //Encourages the badlands generator to use jump line relaxation even if a + b >= c. Set this lower if you want Brazil's jumplines to be more direct, high values might be very wacky. 1.0 will give all of the systems a direct jumpline to rubiconnector. Values below 1 might be very wacky.
...
/datum/star_system/proc/generate_litelands()
	...
	for(var/I=0;I<amount,I++){
		...
		while(!randy_valid)
			randy.x = ((rand(1, 10)/10)+rand(6, 70)) //0 min
			randy.y = ((rand(1, 10)/10)+rand(1, 35)+125) //160 max
			var/syscheck_pass = TRUE
			for(var/datum/star_system/S in (generated + rubicon + src))
				if(!syscheck_pass)
					break
