...
/datum/nsv_mission/proc/send_item(var/contents, var/cargo = TRUE) // Send an item to the players. 
  if(!contents)
    return FALSE
...
/datum/nsv_mission/kill_ships/system/arrive() 
  ...
  if(targets_in_system() <= 2) 
    owner.current_system.spawn_enemies(/obj/structure/overmap/syndicate/ai, 5) // TODO: Look into the faction datum to get types from there
...
/datum/nsv_mission/kill_ships/waves/proc/spawn_new_wave()
  ...
  ships_remaining = ships_to_kill
  owner.current_system.spawn_enemies(/obj/structure/overmap/syndicate/ai,ships_to_kill)// TODO: Look into the faction datum to get types from there
...
/datum/nsv_mission/kill_ships/waves/update_description()
  if(stage == MISSION_COMPLETE)
    desc = "You held [target_system] for [waves_remaining]!"
  if(owner.current_system != target_system)
    desc = "Travel to [target_system] and prepare to fight [waves_remaining] waves of enemy ships"
  if(ships_remaining)  
    ...
    desc = "Wait for enemy renforcements."
