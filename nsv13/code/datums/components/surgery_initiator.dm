...
/datum/component/surgery_initiator/proc/get_available_surgeries(mob/user, mob/living/target)
	...
	for(var/datum/surgery/surgery as anything in GLOB.surgeries_list)
		...
		if(affecting)
			...
			continue
		if(surgery.lying_required && (carbon_target.mobility_flags & MOBILITY_STAND))
			continue
...
/datum/component/surgery_initiator/proc/attempt_cancel_surgery(datum/surgery/the_surgery, mob/living/patient, mob/user)
	...
	user.visible_message(
		"<span class='notice'>[user] closes [patient]'s [parse_zone(selected_zone)] with [close_tool] and removes [parent].</span>",
		"<span class='notice'>You close [patient]'s [parse_zone(selected_zone)] with [close_tool] and remove [parent].</span>",
	)
