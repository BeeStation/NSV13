...
/obj/machinery/computer/ship/salvage/ui_act(action, params)
	...
	switch(action)
		if("salvage")
			...
			can_salvage = FALSE
			DISABLE_BITFIELD(OM.deletion_behavior, DAMAGE_DELETES_UNOCCUPIED) // Simplemobs don't count, so don't let this explode before we're ready
		...
		if("stop_salvage")
			...
			linked.active_boarding_target.kill_boarding_level(linked)
			linked.active_boarding_target.deletion_behavior = initial(linked.active_boarding_target.deletion_behavior)
			linked.active_boarding_target = null
			addtimer(VARSET_CALLBACK(src, can_salvage, TRUE), salvage_cooldown/2)
