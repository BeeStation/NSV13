//Lazy cables for terrible mappers
...
GLOBAL_LIST_INIT(wire_node_generating_types, typecacheof(list(/obj/structure/grille, /obj/structure/cable/cable_bridge)))
...
/obj/structure/cable/auto/proc/connect_wire(clear_before_updating = FALSE)
	...
	for(var/check_dir in GLOB.cardinals)
		...
		for(var/obj/structure/cable/auto/C in TB)
...
/obj/structure/cable/auto/Destroy()					// called when a cable is deleted
...
/obj/structure/cable/auto/deconstruct(disassembled = TRUE)
	if(!(flags_1 & NODECONSTRUCT_1))
		new /obj/item/stack/cable_coil/auto(get_turf(loc), 1)
	qdel(src)
...
/obj/structure/cable/auto/hide(i)
	if(level == 1 && isturf(loc))
		invisibility = i ? INVISIBILITY_MAXIMUM : 0
	update_icon()

/obj/structure/cable/auto/update_icon()
	if(!linked_dirs)
...
/obj/structure/cable/auto/handlecable(obj/item/W, mob/user, params)
...
//
/obj/structure/cable/auto/attackby(obj/item/W, mob/user, params)
	handlecable(W, user, params)


// shock the user with probability prb
/obj/structure/cable/auto/shock(mob/user, prb, siemens_coeff = 1)
	if(!prob(prb))
		return FALSE
...
/obj/structure/cable/auto/singularity_pull(S, current_size)
	..()
...
/obj/structure/cable/auto/add_avail(amount)
	if(powernet)
		powernet.newavail += amount

/obj/structure/cable/auto/add_load(amount)
	if(powernet)
		powernet.load += amount

/obj/structure/cable/auto/surplus()
	if(powernet)
		return CLAMP(powernet.avail-powernet.load, 0, powernet.avail)
	else
		return 0

/obj/structure/cable/auto/avail()
	if(powernet)
		return powernet.avail
	else
		return 0

/obj/structure/cable/auto/add_delayedload(amount)
	if(powernet)
		powernet.delayedload += amount

/obj/structure/cable/auto/delayed_surplus()
	if(powernet)
		return CLAMP(powernet.newavail - powernet.delayedload, 0, powernet.newavail)
	else
		return 0

/obj/structure/cable/auto/newavail()
	if(powernet)
...
/obj/structure/cable/auto/mergeConnectedNetworks(direction)
...
/obj/structure/cable/auto/mergeConnectedNetworksOnTurf()
...
/obj/structure/cable/auto/proc/get_cable_connections(powernetless_only)
	. = list()
	var/turf/T = get_turf(src)
	if(locate(/obj/structure/cable/cable_bridge) in T)
		for(var/obj/structure/cable/auto/C in T)
			if(C != src)
				. += C
	for(var/check_dir in GLOB.cardinals)
		if(linked_dirs & check_dir)
...
/obj/structure/cable/auto/proc/get_machine_connections(powernetless_only)
	. = list()
...
/obj/structure/cable/auto/auto_propogate_cut_cable(obj/O)
	if(O && !QDELETED(O))
...
/obj/structure/cable/auto/proc/propogate_if_no_network()
	if(powernet)
		return
	var/datum/powernet/newPN = new()
...
/obj/structure/cable/auto/cut_cable_from_powernet(remove = TRUE)
	if(!powernet)
		return
	...
	for(var/dir_check in GLOB.cardinals)
		if(linked_dirs & dir_check)
			...
			P_list += locate(/obj/structure/cable/auto) in T1
...
/obj/item/stack/cable_coil/auto
	name = "lazy cable coil"
	desc = "Triggering engineering mains since 2256."
	custom_price = 30

/obj/item/stack/cable_coil/auto/Initialize(mapload, new_amount = null)
	. = ..()
	pixel_x = rand(-2,2)
	pixel_y = rand(-2,2)
	update_icon()
...
/obj/item/stack/cable_coil/auto/examine(mob/user)
	. = ..()
	. += "<b>Ctrl+Click</b> to change the layer you are placing on."

/obj/item/stack/cable_coil/auto/suicide_act(mob/user)
...
/obj/item/stack/cable_coil/auto/proc/check_menu(mob/living/user)
	if(!istype(user))
		return FALSE
...
/obj/item/stack/cable_coil/auto/CtrlClick(mob/living/user)
	if(!user)
		return
	...
	switch(layer_result)
		if("Layer 1")
			color = "red"
			target_type = /obj/structure/cable/auto/layer1
		if("Layer 2")
			color = "yellow"
			target_type = /obj/structure/cable/auto
		if("Layer 3")
			color = "blue"
			target_type = /obj/structure/cable/auto/layer3
...
/obj/item/stack/cable_coil/auto/attack(mob/living/carbon/human/H, mob/user)
	if(!istype(H))
		return ..()
...
/obj/item/stack/cable_coil/auto/update_icon()
	icon_state = "[initial(item_state)][amount < 3 ? amount : ""]"
	name = "cable [amount < 3 ? "piece" : "coil"]"

//add cables to the stack
/obj/item/stack/cable_coil/auto/give(extra)
...
/obj/item/stack/cable_coil/auto/place_turf(turf/T, mob/user, dirnew)
	if(!isturf(user.loc))
		return
	...
	var/obj/structure/cable/auto/C = new target_type(T)
	...
	if(C.shock(user, 50))
		if(prob(50)) //fail
			new /obj/item/stack/cable_coil/auto(get_turf(C), 1)
			C.deconstruct()

	return C

/obj/item/stack/cable_coil/auto/five
	amount = 5

/obj/item/stack/cable_coil/auto/cut
	amount = null
	icon_state = "coil2"

/obj/item/stack/cable_coil/auto/cut/Initialize(mapload)
	. = ..()
	if(!amount)
		amount = rand(1,2)
	pixel_x = rand(-2,2)
...
/obj/item/stack/cable_coil/auto/cyborg
	is_cyborg = 1
	materials = list()
	cost = 1

/obj/structure/cable/cable_bridge
	name = "cable bridge"
...
/obj/structure/cable/cable_bridge/Initialize()
	. = ..()
	var/first = TRUE
	var/datum/powernet/PN
	for(var/obj/structure/cable/auto/C in get_turf(src))
		C.update_icon()
		if(first == TRUE)
			first = FALSE
			PN = C.powernet
...
/obj/structure/cable/cable_bridge/wirecutter_act(mob/living/user, obj/item/I)
	. = ..()
	qdel(src)
