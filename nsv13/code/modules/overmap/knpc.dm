...
/datum/component/knpc/proc/pathfind_to(atom/movable/target, turf/avoid)
	...
	if(target)
		...
		path = get_path_to(H, dest, max_distance = 120, 0, id=access_card, simulated_only=!(H.wear_suit?.clothing_flags & STOPSPRESSUREDAMAGE && H.head?.clothing_flags & STOPSPRESSUREDAMAGE), exclude=avoid)
