...
/* Disabled until I can figure out how to make it work with our Radiation system
...
/obj/item/mod/module/flashlight/generate_worn_overlay(mutable_appearance/standing)
	...
	if(!active)
		return
	var/mutable_appearance/light_icon = mutable_appearance('nsv13/icons/mob/mod.dmi', "module_light_on", layer = standing.layer + 0.2)
...
/* Not useful for anything as far as I know, so disabled for now
...
/obj/item/projectile/organ/on_hit(atom/target)
	...
	if(succeed)
		...
		for(var/obj/item/organ/organ_evacced as anything in organs_to_boot_out)
			if(organ_evacced.organ_flags & ORGAN_VITAL)
				continue
...
/obj/item/implant/mod/Initialize(mapload)
	...
	if(!istype(loc, /obj/item/mod/module/pathfinder))
		return INITIALIZE_HINT_QDEL
	module = loc
	jet_icon = image(icon = 'nsv13/icons/obj/mod.dmi', icon_state = "mod_jet", layer = LOW_ITEM_LAYER)
