...
/datum/robot_control/ui_interact(mob/user, datum/tgui/ui)
	...
	if(!ui)
		...
		ui.open()
		ui.set_autoupdate(TRUE) //Patrol Positions and Status Updates
...
/datum/robot_control/ui_data(mob/user)
	...
	for(var/mob/living/simple_animal/bot/B in GLOB.bots_list)
		if((B.get_virtual_z_level() in SSmapping.levels_by_trait(valid_z)) && !B.remote_disabled) //Only non-emagged bots on the same ship are detected!
