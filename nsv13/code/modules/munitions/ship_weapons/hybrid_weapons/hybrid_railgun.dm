...
/obj/machinery/ship_weapon/hybrid_rail/proc/try_use_power(amount) //Pulling power from wires rather then APCs
	...
	if(C)
		if(!C.powernet)
			return FALSE
		available_power = C.powernet.avail-C.powernet.load

		if(available_power && available_power >= amount)
...
/obj/machinery/ship_weapon/hybrid_rail/ui_interact(mob/user, ui_key = "main", datum/tgui/ui = null, force_open = FALSE, \
	...
	if(!ui)
		ui = new(user, src, ui_key, "HybridWeapons", name, 600, 350, master_ui, state)
		ui.open()
...
/obj/machinery/ship_weapon/hybrid_rail/ui_act(action, params)
	...
	if(action == "power_allocation")
		if(adjust && isnum(adjust))
			...
			if(capacitor_current_charge_rate > capacitor_max_charge_rate)
				capacitor_current_charge_rate = capacitor_max_charge_rate
				return
