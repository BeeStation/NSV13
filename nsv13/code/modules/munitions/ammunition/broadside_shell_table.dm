/obj/machinery/broadside_shell_packer
	name = "\improper Broadside Shell Packer"
...
/obj/machinery/broadside_shell_packer/examine(mob/user)
	. = ..()
	. += "It has [shell_amount] shells, [projectile_amount] projectiles, and [bag_amount] bags."
	. += "The Packer requires 5 Shells, 5 Projectiles and 1 Powder Bag to pack 5 Broadside Shells."
...
/obj/machinery/broadside_shell_packer/attackby(obj/item/I, mob/living/user, params)
	...
	if(thingy[I.type])
		...
		if(istype(I, /obj/item/ship_weapon/parts/broadside_projectile))
			//if(projectile_amount == 0)
			//add_overlay("broadside_projectile")
			if(projectile_amount < amount_to_pack)
				projectile_amount++
				to_chat(user, "<span class='notice'>You add a projectile to the table.</span>")
				I.forceMove(src)
				qdel(I)
			else
				to_chat(user, "<span class='warning'>The table is already full of projectiles!</span>")
...
/obj/machinery/broadside_shell_packer/attack_hand(mob/living/user)
	. = ..()
	if(shell_amount == amount_to_pack && projectile_amount == amount_to_pack && bag_amount == 1)
		//cut_overlays()
		...
		if(projectile_amount < amount_to_pack)
			to_chat(user, "<span class='warning'>The table is missing [amount_to_pack - projectile_amount] projectiles!</span>")
		if(bag_amount < 1)
