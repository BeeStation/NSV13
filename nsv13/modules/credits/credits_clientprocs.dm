...
/client/proc/download_credits()
	...
	if(!end_credits.finalized)
		log_runtime("[src] tried to download credits before the credits were ever finalized! Credits preference: [prefs.credits]")
		return*/
...
/client/proc/play_downloaded_credits()
	...
	if(!received_credits)
		log_runtime("[src] tried to play credits without having ever received them! Credits preference:")// INSERT BACK IN [prefs.credits]
		return
...
/client/proc/jingle_audio(var/preload_only = FALSE)
	/*if(prefs.credits == CREDITS_ALWAYS)
		return*/
	var/blerg = pick(JINGLE_NEVER,JINGLE_CLASSIC,JINGLE_ALL)
	var/link = ""
	switch(blerg)
		if(JINGLE_NEVER)
			return
	...
	if(!link)
		log_runtime("[src] somehow had a null jingle link! Jingle preference: ")//[prefs.jingle]
