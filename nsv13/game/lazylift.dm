...
/obj/machinery/lazylift/master/proc/path_to(target_deck, startDeck=platform_location.deck)
	...
	if(!path.len)
		...
		in_use = FALSE //in use more like in PUCE am I right gamers???
		return FALSE //FUCK
	platform_location.say("Turbolift engaging. Please stand by.")
...
/obj/machinery/lazylift/master/proc/sound_effect(start)
	if(start)
		for(var/mob/M in get_area(src))
			SEND_SOUND(M, 'nsv13/sound/effects/lift/lift_start.ogg')
			shake_camera(M, 2, 1)
			if(!isliving(M))
				continue
			if(obj_flags & EMAGGED)
		...
		sleep(1.4 SECONDS) //Sound time!
		for(var/mob/M in get_area(src))
			SEND_SOUND(M, sound('nsv13/sound/effects/lift/lift_loop.ogg', repeat = TRUE, wait = 0, volume = 100, channel = CHANNEL_AMBIENCE))
		return
	else
		for(var/mob/M in get_area(src))
			SEND_SOUND(M, sound('nsv13/sound/effects/lift/lift_end.ogg', repeat = FALSE, wait = 0, volume = 100, channel = CHANNEL_AMBIENCE))
		sleep(2 SECONDS)
		return
