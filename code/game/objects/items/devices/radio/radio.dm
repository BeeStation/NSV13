...
/obj/item/radio/proc/talk_into_impl(atom/movable/M, message, channel, list/spans, datum/language/language, list/message_mods)
	...
	*/
	...
	if (independent && (freq == FREQ_CENTCOM || freq == FREQ_CTF_RED || freq == FREQ_CTF_BLUE || freq == FREQ_OVERMAP_NANOTRASEN || freq == FREQ_OVERMAP_SYNDICATE || freq == FREQ_OVERMAP_PIRATE || freq == FREQ_PIRATE)) //Nsv13 - atc channels & pirate
...
/obj/item/radio/proc/can_receive(freq, level)
	...
	if (freq == FREQ_SYNDICATE && !(syndie || hearall))
		return FALSE
	if (freq == FREQ_CENTCOM || freq == FREQ_OVERMAP_NANOTRASEN || freq == FREQ_OVERMAP_SYNDICATE || freq == FREQ_OVERMAP_PIRATE || freq == FREQ_PIRATE) //NSV13
