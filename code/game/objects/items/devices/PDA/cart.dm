...
/obj/item/cartridge/proc/bot_control()
	if(active_bot)
		...
		for(var/B in GLOB.bots_list) //Git da botz
			...
			if(!Bot.on || Bot.remote_disabled || !(bot_access_flags & Bot.bot_type)) //Only non-emagged bots are detected! //NSV13
				continue //Also, the PDA must have access to the bot type.
			if(Bot.get_virtual_z_level() in SSmapping.levels_by_trait(ZTRAIT_STATION)) //NSV13
				if(zlevel in SSmapping.levels_by_trait(ZTRAIT_STATION)) //NSV13
					menu += "<A href='byond://?src=[REF(src)];op=control;bot=[REF(Bot)]'><b>[Bot.name]</b> ([Bot.get_mode()])<BR>" //NSV13
					botcount++ //NSV13
			else if(Bot.get_virtual_z_level() == zlevel) //NSV13
				if(!(zlevel in SSmapping.levels_by_trait(ZTRAIT_STATION))) //NSV13
					menu += "<A href='byond://?src=[REF(src)];op=control;bot=[REF(Bot)]'><b>[Bot.name]</b> ([Bot.get_mode()])<BR>" //NSV13
					botcount++ //NSV13
