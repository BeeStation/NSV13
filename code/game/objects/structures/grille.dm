...
/obj/structure/grille/rcd_vals(mob/user, obj/item/construction/rcd/the_rcd)
	switch(the_rcd.mode)
		...
		if(RCD_WINDOWGRILLE)
			if(the_rcd.window_type == /obj/structure/window/reinforced/fulltile/ship/interior)
		...
		if (!broken)
			...
			if(do_after(user,20, target = src))
				...
				if(istype(W, /obj/item/stack/sheet/plasmarglass))
					...
					WD = new/obj/structure/window/reinforced/fulltile/ship/interior(drop_location()) //reinforced window
