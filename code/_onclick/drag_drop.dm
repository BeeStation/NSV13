...
/mob/living/canMobMousedown(atom/object, location, params) //NSV13 - allow non-carbons to do this too
	...
	else if(src.overmap_ship && (src.overmap_ship.gunner == src || src.overmap_ship.pilot == src) || (GetComponent(/datum/component/overmap_gunning))) //NSV13 - let us mouse-down if we're a ship operator
		. = src.overmap_ship
