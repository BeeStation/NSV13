...
/datum/disease/advance/proc/random_disease_name(var/atom/diseasesource)//generates a name for a disease depending on its symptoms and where it comes from
	var/list/prefixes = list("Spacer's ", "Space ", "Infectious ","Viral ", "The ", "[human_first_name_random()]'s ", "[human_last_name_random()]'s ", "Acute ")//prefixes that arent tacked to the body need spaces after the word
	var/list/bodies = list(pick("[human_first_name_random()]", "[human_last_name_random()]"), "Space", "Disease", "Noun", "Cold", "Germ", "Virus")
