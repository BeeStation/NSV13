	...
	for(var/obj/machinery/door/poddoor/M in GLOB.machines)
		if(M.id == src.id)
			if(openclose == null || !sync_doors)
				openclose = M.density
			INVOKE_ASYNC(M, openclose ? PROC_TYPE_REF(/obj/machinery/door/poddoor, open) : TYPE_PROC_REF(/obj/machinery/door/poddoor, close))
...
/obj/item/assembly/control/airlock/activate()
	...
	for(var/D in open_or_close)
		INVOKE_ASYNC(D, doors_need_closing ? PROC_TYPE_REF(/obj/machinery/door/airlock, close) : TYPE_PROC_REF(/obj/machinery/door/airlock, open))
