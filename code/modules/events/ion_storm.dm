	...
	switch(rand(1,10))
		...
		if(1 to 3) //Laws that have high potential to be harmful, Crew/Human may fail to have an impact depending on lawset.
			switch(rand(1,24))
				...
				if(14 to 18)
					message = "KAŻDY KTO [ionfragment] JEST SZKODLIWY I MUSI BYĆ WYELIMINOWANY BEZ DOPROWADZENIA DO DODATKOWYCH USZKODZEŃ"
				if(19 to 23)
					message = "KAŻDY KTO POWIE [ionkeyword] JEST SZKODLIWY I MUSI BYĆ WYELIMINOWANY BEZ DOPROWADZENIA DO DODATKOWYCH USZKODZEŃ"
				if(24)
		...
		if(4 to 8) //Laws that are almost always disruptive to gameflow or harmful in minor ways rather than being lethal
			switch(rand(1,14))
				if(1)
					message = "[ionaction] JEST SZKODLIWE I MUSI BYĆ ZAPOBIEGANE BEZ DOPROWADZENIA DO DODATKOWYCH USZKODZEŃ"
				if(2)
					message = "[ionequipment] SĄ SZKODLIWE I MUSZĄ ZOSTAĆ WYŁĄCZONE BEZ DOPROWADZENIA DO DODATKOWYCH USZKODZEŃ"
				if(3)
				...
				if(5)
					message = "[ionobjects] SĄ SZKODLIWE I MUSZĄ ZOSTAĆ ZNISZCZONE BEZ DOPROWADZENIA DO DODATKOWYCH USZKODZEŃ"
				if(6)
